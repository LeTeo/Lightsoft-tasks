...
$text = "Sum summus mus";//строка текста


function strToLowerWithoutSpaces($str)
{
...
if($text == "" || $text == null)
	echo "Введена пустая строка или не определена!";
else
{
	$manachersAlgorithm = new ManachersAlgorithm;
	$result = $manachersAlgorithm->findLongestPalindrome($text);

	if(strToLowerWithoutSpaces($text) == $result)
		echo($text);
	else if(mb_strlen($result) == 1)
